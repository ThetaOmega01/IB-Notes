...
\begin{method}[`Exact' formulae]
    ...
    \begin{itemize}
        \item if $N>n$, then it is not a polynomial of degree $n$ that substitutes the function, since if we write RHS as a polynomial then there are $ \le n+1 $ free coefficients: $(a_i)_{i=0}^{N}$ is redundant;
